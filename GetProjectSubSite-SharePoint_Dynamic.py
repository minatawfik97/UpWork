...
def pull_data_from_sharepoint(sub_site, list_name):
    _service_document_location = f"{_site}/{sub_site}/_api/web/lists/getbytitle('{list_name}')/items"
    ...
    for entry in root.iter('{http://www.w3.org/2005/Atom}entry'):
        ...
        for elem in entry.iter():
            if elem.tag.startswith('{http://schemas.microsoft.com/ado/2007/08/dataservices}') or elem.tag.endswith('title'):
                if elem.tag.endswith('element'):  # Skip the 'element' column
                    continue
                value = elem.text

                # Check if the element has child elements
                child_elements = list(elem.iter())
                if len(child_elements) > 1:
                    # Concatenate values of child elements separated by ':::'
                    value = ':::'.join(child.text for child in child_elements[1:])
...
all_projects = Get_project_Data("SELECT ProjectId, SUBSTRING(ProjectWorkspaceInternalUrl, CHARINDEX('/VRO/', ProjectWorkspaceInternalUrl) + LEN('/VRO/'), LEN(ProjectWorkspaceInternalUrl)) AS Sub_Site \
                                FROM [Pre].[ProjectsByTaples]")

all_data = pd.DataFrame()
combined_df = pd.DataFrame()
not_found_df = pd.DataFrame(columns=['Project', 'List'])
...
for list_name in all_lists:
    ...
    for project, sub_site in all_projects.items():
        #print(f"{list_name} >> {project} >> {sub_site}")
        try:
            df = pull_data_from_sharepoint(sub_site, list_name)
            df['PID'] = project
            if df.empty:
            ...
            df_list.append(df)
            print(f"Data for list {list_name} fetched successfully from sub_site {sub_site}")
            
        except:
            print(f"Error fetching data for list {list_name}")
            print(f"No data found for list {list_name} on SharePoint in sub_site {sub_site}. Skipping...")
            not_found_df = not_found_df._append({'Project': project, 'List': list_name}, ignore_index=True)
    ...
    if len(df_list) > 0:
        ...
        #file_path = f'D:\MOF_Migration\Lists\{list_name}_Projects.csv'
        #merged_df.to_csv(file_path, index=False, mode='w')
        create_table_from_csv(merged_df, list_name, 'PSList')
        insert_data_into_table(merged_df, list_name, 'PSList')
        print(f"List {list_name} migrated successfully into PSList.{list_name}")

if not_found_df.empty:
    print("All lists found for all projects.")
else:
    insert_data_into_table(not_found_df, 'Not_Founded_PSList', 'dbo')
    print("Lists not found for projects inserted into the 'Not_Founded_PSList' table.")