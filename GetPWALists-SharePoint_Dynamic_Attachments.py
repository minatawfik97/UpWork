...
def pull_data_from_sharepoint(list_name):
    _service_document_location = f"{_site}/_api/web/lists/getbytitle('{list_name}')/items?$expand=AttachmentFiles&$select=AttachmentFiles&$Top=5000"
...
for list_name in all_lists:
    ...
    try:
        ...
        err_msg = f"Error migrating list {list_name}: {str(e)}"
        print(err_msg)