...
def Get_Lookup_Name_Table():
    _conn = pyodbc.connect(conn_str)
    cursor = _conn.cursor()
    _query = f"SELECT DISTINCT _Source, \
                        'Mapping.Unified_Mapping' AS Table_Name, Destination, \
                        CASE WHEN Destination = 'ChangeRequestsStudies' THEN 2 ELSE 1 END AS _RN \
                FROM Mapping.Unified_Mapping \
                WHERE Table_Type = 'Project List' \
                AND _Source NOT IN ('KPIs') \
...
for _Source_name, (_table, _dest) in _dict_lookup.items():
    df = _query_data(f"SELECT * FROM GoLive.[Ps_{_Source_name}]")
    ...
    mapping_df = _query_data(f"SELECT DISTINCT \
                                ISNULL([Internal Name], REPLACE([Display Name], ' ', '')) AS [Internal Name], \
                                        [Display Name1] AS [Internal Name1] \
                                FROM {_table} \
                                WHERE [Display Name] NOT IN ('NULL') \
                                        AND [Display Name1] <> 'NULL' \
                                        AND Table_Type = 'Project List' \
                                        AND _Source= '{_Source_name}' \
        ...
        try:
            if item_data:
                ...
                if response['1,New'] == '0x00000000':
                    ...
                    print(f"Data upload failed for {_dest}>> {source_column}>> {destination_column} : {response} ")
                    _df_errors = _df_errors._append({'_Error': str(response), '_List': _dest, '_Type' : 'Project'}, ignore_index=True)
                    break
        except Exception as e:
            print(f"Error occurred for {_dest}>> Column Source [{source_column}]>> Column destination [{destination_column}]: {e}")
            _df_errors = _df_errors._append({'_Error': str(e), '_List': _dest, '_Type' : 'Project'}, ignore_index=True)
            continue
    df = spid._Get_SPID(site_url, list_name)
    df['PID'] = list_name
    df['Type'] = 'Project List'
