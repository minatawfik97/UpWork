...
def pull_data_from_sharepoint(url):
    _service_document_location = f"{_site}/_api/web/GetFileByServerRelativeUrl(@url)/ListItemAllFields?@url='{url}'"
...
for list_name in all_lists:
    ...
    all_projects = _query_data(f"SELECT DISTINCT T2.serverrelativeurl \
			                    ,   T2.[name] \
                                FROM DOC_RTLvl.[Documents] T2")
    ...
    for index, row in all_projects.iterrows():
        ...
        try:
            ...
            #print(encoded_url)
            df = pull_data_from_sharepoint(encoded_url)
            ...
            df_list.append(df)
            print(f"File {row['name']} metadata from list {list_name} fetched successfully...")
        
        except:
            print(f"Error fetching data for list {list_name} @ {row['name']}")
            print(f"No data found for list {list_name} on SharePoint @ {row['name']}. Skipping...")
    ...
    if len(df_list) > 0:
        ...
        create_table_from_csv(merged_df, f"{list_name}_metadata", 'DOC_RTLvl')
        insert_data_into_table(merged_df, f"{list_name}_metadata", 'DOC_RTLvl')
