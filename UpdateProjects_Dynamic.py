...
for _name, (_table, _dest) in _dict_lookup.items():
    query = "SELECT [Destination Stage], [Destination Phase], T2._SPID \
                FROM Project.PROJECTS T1 \
                    INNER JOIN Trace_SPID T2 \
                        ON T1.title = T2._Title \
                            WHERE T1.[Destination Stage] IS NOT NULL"
    ...
    for index, row in query_result.iterrows():
        try:
            ...
            item_id = row['_SPID']
            stages = row['Destination Stage']
            phases = row['Destination Phase']

            if pd.notnull(item_id) and pd.notnull(stages) and pd.notnull(phases):
