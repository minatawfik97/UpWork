...
def upload_file(site, auth, target_folder_path, file_name, file_content, spid, pcode, title, msr_path, _url):
    try:
        request_digest = get_request_digest(site.site_url, auth)

        upload_url = f"{site.site_url}/_api/web/getfolderbyserverrelativeurl('{urllib.parse.quote(target_folder_path)}')/files/add(url='{file_name}', overwrite=false)"
        ...
        if response.status_code == 200:
            ...
            doc_library = site.List("Projects Documents")
...
for _table in doc_sites:
    ...
    attachmets = _query_data(f"SELECT DISTINCT \
                            ...
                            ,	T2._SPID \
                            ,	T1.Title \
                            ,   IIF(T2._SPID IS NOT NULL, CONCAT( \
								SUBSTRING(serverrelativeurl, CHARINDEX('/Attachments/', serverrelativeurl) + LEN('/Attachments/'), \
								...
								'_', [filename]), [filename]) _filename \
                            FROM {_table}.[List_Attachments] T1 \
                            INNER JOIN Trace_SPID T2 \
                            ON T1.PCODE = T2._Title \
                            WHERE T1.Main_Folder IS NOT NULL \
                            AND CONCAT(T1.Main_Folder, '/', T1.Sub_Folder, '/', T1.[filename]) IN \
                            ( SELECT _Title From Trace_SPID_Attachments_failed \
                            WHERE _Type = 'List attatchments')")
    ...
    for index, row in attachmets.iterrows():
        ...
        try:
            response, max_id = upload_file(site, auth, target_folder_path, file_name, file_content, _spid, _pcode, _title, _msr_path, encoded_url)

            data_to_append = {
            '_pcode': folder_name + '/' + file_name,
            '_spid': max_id,
            '_Entity': "Projects Documents",
