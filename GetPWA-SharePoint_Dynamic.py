...
def pull_data_from_sharepoint(list_name):
    _service_document_location = f"{_site}/_api/web/lists/getbytitle('{list_name}')/items?$top=50000"
...
all_lists = query_data("SELECT DISTINCT _Source \
                        ...
                        AND REPLACE(_Source, ' ', '') NOT IN ('ProjectStages', 'مزوديالخدمة') \
                        ORDER BY _Source")
...
for list_name in all_lists:
    ...
    try:
        ...
        #file_path = f'{_partition_name}{_folder}lists\\{list_name}.csv'
        #df.to_csv(file_path, index=False, mode='w')
        _listname = list_name.replace(" ", "")
        create_table_from_csv(df, _listname, "dbo")
        insert_data_into_table(df, _listname, "dbo")
        print(f"List {_listname} migrated successfully")
    except Exception as e:
        not_found_lists.append(_listname)
        err_msg = f"Error migrating list {_listname} : {str(e)}"
        print(err_msg)

if not_found_lists:
    not_found_df = pd.DataFrame({"_Name": not_found_lists})
    insert_data_into_table(not_found_df, "Not_Founded_Lists", "dbo")
    print("Lists not found on SharePoint inserted into Not_Founded_Lists table")