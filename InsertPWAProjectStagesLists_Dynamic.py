...
def Get_Lookup_Name_Table():
    ...
    _query = f"SELECT DISTINCT REPLACE(_Source, ' ' , '') AS _Source, \
                ...
                WHERE Table_Type = 'PWA List' \
                    ...
                    AND _Source NOT IN ('KPIs', 'OrgHealthKPIs') \
...
for _Source_name, (_table, _dest) in _dict_lookup.items():
    df = _query_data(f"SELECT * FROM GoLive.{_Source_name}\
                     ORDER BY [projectid], N_Stage, N_Phase")
    ...
    mapping_df = _query_data(f"SELECT DISTINCT REPLACE([Display Name], ' ', '') AS [Internal Name], \
                                ...
                                WHERE [Display Name] NOT IN ('NULL') \
                                        ...
                                        AND REPLACE(_Source, ' ', '') NOT IN ('KPIs', 'OrgHealthKPIs') \
        ...
        try:
            if item_data:
                ...
                if response['1,New'] == '0x00000000':
                    ...
                    _df_errors = _df_errors._append({'_Error': str(response), '_List': _dest, '_Type' : 'ProjectStages'}, ignore_index=True)
                    break
        except Exception as e:
            print(f"Error occurred for {_dest}>> Column Source [{source_column}]>> Column destination [{destination_column}]: {e}")
            _df_errors = _df_errors._append({'_Error': str(e), '_List': _dest, '_Type' : 'ProjectStages'}, ignore_index=True)
            continue
