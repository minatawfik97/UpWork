...
def pull_data_from_sharepoint(list_name):
    _service_document_location = f"{_site}/_api/web/lists/getbytitle('{list_name}')/items?$expand=File&$select=File&$filter=ContentType ne 'Folder'&$Top=5000"
...
for list_name in all_lists:
    ...
    try:
        ...
        df_list.append(df)
        print(f"Data for list {list_name} fetched successfully...")
...
if len(df_list) > 0:
    ...
    create_table_from_csv(merged_df, list_name, 'DOC_RTLvl')
    insert_data_into_table(merged_df, list_name, 'DOC_RTLvl')
    print(f"List {list_name} migrated successfully into DOC_RTLvl.{list_name}")
