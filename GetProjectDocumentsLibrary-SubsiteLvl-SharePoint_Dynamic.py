...
def pull_data_from_sharepoint(sub_site, list_name):
    _service_document_location = f"{_site}/{sub_site}/_api/web/lists/getbytitle('{list_name}')/items?$expand=File&$select=File&$filter=ContentType ne 'Folder'&$Top=5000"
...
all_lists = ['Documents', 'ضبط الوثائق']
...
for list_name in all_lists:
    ...
    if len(df_list) > 0:
        ...
        #file_path = f'D:\MOF_Migration\Lists\{list_name}_DOCProjects.csv'
        #merged_df.to_csv(file_path, index=False, mode='w')
        create_table_from_csv(merged_df, list_name, 'DOC_PSLvl')
        insert_data_into_table(merged_df, list_name, 'DOC_PSLvl')
...
if not_found_df.empty:
    ...
    insert_data_into_table(not_found_df, 'Not_Founded_DocPSList', 'dbo')
    print("Lists not found for projects inserted into the 'Not_Founded_DocPSList' table.")