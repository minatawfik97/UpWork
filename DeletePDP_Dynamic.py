...
def Get_Lookup_Name_Table():
    ...
    cursor = _conn.cursor()
    _query = f"SELECT DISTINCT CASE \
                        WHEN Destination = 'Gates' THEN '[GoLive].[Gates]' \
                        WHEN Destination = 'GatesResult' THEN '[GoLive].[GatesResults]' \
                        WHEN Destination = 'Contracts' THEN '[GoLive].[Contracts]' \
                        WHEN Destination = 'Benefits' THEN '[GoLive].[Benefits_PDP]' \
                        WHEN Destination = 'RolesAndResponsabilities' THEN '[GoLive].[RolesAndResponsabilities]' \
                        END AS [Name], \
                    'Mapping.Unified_Mapping' AS Table_Name, Destination \
                FROM Mapping.Unified_Mapping \
                WHERE Table_Type = 'PDP' AND Destination <> 'Projects'"
...
def delete_from_trace_spid(_list, _dst):
    conn = pyodbc.connect(conn_str)
    cursor = conn.cursor()
    cursor.execute(f"DELETE FROM [Trace_SPID] WHERE _Type = 'PDP2' AND _Entity = '{_dst}' AND _SPID IN ({','.join(_list)})")
    conn.commit()
    cursor.close()
    conn.close()
...
def delete_items_from_sharepoint_list(_dest):
    ...
    while True:
        ...
        if df_delete.empty:
            ...
            delete_from_trace_spid(id_list, _dest)
