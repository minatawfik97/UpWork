...
def upload_file(site, auth, target_folder_path, file_name, file_content, spid, pcode, title, msr_path, _RelatedPhase, _id, _createby, _editby, _createdate, _editdate, _lib, _url):
    ...
    if response.status_code == 200:
        ...
        meta_update = doc_library.UpdateListItems(data=[
            {
                'ID': max_id,
                'ItemID': spid,
                'ProjectCode': pcode,
        ...
        print(meta_update)
        if meta_update['1,Update'] == '0x00000000':
            print(f"Metadata for '{file_name}' updated successfully.")
        else:
            max_id = "Metadata Update failed"
            print(max_id)
...
for _table in doc_sites:
    ...
    attachmets = _query_data(f"SELECT DISTINCT \
                ...
                LEFT JOIN Not_Migrated_Doc T5 \
		            ON T4._filename = T5.[file_name] AND T4._Doc = T5.[Library] \
		        WHERE T5.[file_name] IS NULL AND T4._filename = 'أمر التعميد MTFF.jpg'")
