...
for _table in doc_sites:
    ...
    attachmets = _query_data(f"SELECT X.* \
                                FROM ( SELECT serverrelativeurl \
                                ...
                                ,	Modified \
                                ,	id \
                            FROM Transformed_RTAttachments)X \
							INNER JOIN Trace_SPID_Attachments_failed T6 \
                ON CONCAT(Dest, '/', REPLACE([name], '''', '')) = T6._Title \
            WHERE T6._Response <> '<Response [200]>' AND T6._Type IN ('Root Document attatchments')")
