...
def Get_Lookup_Name_Table():
    _conn = pyodbc.connect(conn_str)
    cursor = _conn.cursor()
    _query = f"SELECT DISTINCT '[GoLive].[Users_Log_Final]' AS [Name], \
                    'Mapping.Unified_Mapping' AS Table_Name, 'MigrationUserLogs' Destination  \
                    FROM Mapping.Unified_Mapping \
                    WHERE Destination = 'MigrationUserLog'"
    cursor.execute(_query)
    results = cursor.fetchall()
    dict_lookup = {}
    for row in results:
...
for _name, (_table, _dest) in _dict_lookup.items():
    ...
    mapping_df = _query_data(f"SELECT DISTINCT REPLACE([Internal Name], ' ', '') AS [Internal Name], [Display Name1] AS [Internal Name1] \
                                FROM Mapping.Unified_Mapping \
                                WHERE  Destination = 'MigrationUserLog' \
        ...
        try:
            if item_data:
                ...
                if response['1,New'] == '0x00000000':
                    ...
                    _df_errors = _df_errors._append({'_Error': str(response), '_List': _dest, '_Type' : 'Users'}, ignore_index=True)
        except Exception as e:
            print(f"Error occurred for {_dest}>> Column Source [{source_column}]>> Column destination [{destination_column}]: {e}")
            _df_errors = _df_errors._append({'_Error': str(e), '_List': _dest, '_Type' : 'Users'}, ignore_index=True)
            continue
