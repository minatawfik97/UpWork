...
for _table in doc_sites:
    mainfolder = create_folders(_partition_name, _folder, _table)

    doc_files = _query_data(f"SELECT serverrelativeurl, pid, _filename \
                                FROM [dbo].[Doc_Attachments] \
                                WHERE _Doc = '{_table}'")
    ...
    for index, row in doc_files.iterrows():
        ...
        local_download_directory = create_folders(_partition_name, f"{_folder}\{_table}", row['pid'])
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                not_migrated_df = not_migrated_df._append({'Library': _table, 'file_name': file_name, 'file_url': file_path}, ignore_index=True)
