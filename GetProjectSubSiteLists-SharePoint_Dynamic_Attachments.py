...
def pull_data_from_sharepoint(sub_site, list_name):
    _service_document_location = f"{_site}/{sub_site}/_api/web/lists/getbytitle('{list_name}')/items?$expand=AttachmentFiles&$select=AttachmentFiles&$Top=5000"
...
for list_name in all_lists:
    ...
    if len(df_list) > 0:
        ...
        #file_path = f'D:\MOF_Migration\Lists\{list_name}_Projects_Attachments.csv'
        #merged_df.to_csv(file_path, index=False, mode='w')
        create_table_from_csv(merged_df, f"{list_name}_Attachments", 'DOC_PSLvl')
        insert_data_into_table(merged_df, f"{list_name}_Attachments", 'DOC_PSLvl')
        print(f"List {list_name}__Attachments migrated successfully into DOC_PSLvl.{list_name}_Attachments")
